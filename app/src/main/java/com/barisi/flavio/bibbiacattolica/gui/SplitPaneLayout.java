...
public class SplitPaneLayout extends ViewGroup {
    ...
    private Rect mSplitterRectTouch = new Rect();
    ...
    private void extractAttributes(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            if (value != null) {
                ...
                mSplitterDraggingDrawable = new PaintDrawable(R.color.nero);
                mSplitterDraggingDrawable.setColorFilter(Preferenze.coloreAccent(context), PorterDuff.Mode.SRC_ATOP);
                mSplitterDraggingDrawable.setAlpha(220);
            }
            a.recycle();
        }
    }
        ...
        if (widthSize > 0 && heightSize > 0) {
            switch (mOrientation) {
                ...
                case 1: {
                    ...
                    getChildAt(0).measure(MeasureSpec.makeMeasureSpec(widthSize, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(mSplitterPosition - 0, MeasureSpec.EXACTLY));
                    getChildAt(1).measure(MeasureSpec.makeMeasureSpec(widthSize, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(heightSize - mSplitterSize - mSplitterPosition, MeasureSpec.EXACTLY));
                    break;
                }
            }
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        switch (mOrientation) {
            ...
            case 1: {
                getChildAt(0).layout(0, 0, w, mSplitterPosition - 0);
                mSplitterRect.set(0, mSplitterPosition, w, mSplitterPosition + mSplitterSize);
                mSplitterRectTouch.set(0, mSplitterPosition - 2 * mSplitterSize, w, mSplitterPosition + mSplitterSize + 2 * mSplitterSize);
                getChildAt(1).layout(0, mSplitterPosition + mSplitterSize, w, h);
                break;
            }
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if (mSplitterRectTouch.contains(x, y) && mSplitterPositionPercent < 1.0f) {
            return true;
        }
        return false;
    }
        ...
        if (mSplitterMovable) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN: {
                    if (mSplitterRectTouch.contains(x, y)) {
                        isDragging = true;
                    ...
                    if (isDragging) {
                        switch (mOrientation) {
                            ...
                            case ORIENTATION_VERTICAL: {
                                ...
                                int limitinf = (int) (getHeight() * 0.3);
                                int limitsup = getHeight() - mSplitterSize + 1;
                                if (temp.top < limitinf || temp.top > limitsup) {
                                    temp.top = top;
                                    temp.bottom = bottom;
                                }
                                break;
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        setSplitterPositionPercent(mSplitterPositionPercent);
    }
    ...
    public void setColor(int color) {
        mSplitterDraggingDrawable.setColorFilter(color, PorterDuff.Mode.SRC_ATOP);
        mSplitterDrawable.setColorFilter(color, PorterDuff.Mode.SRC_ATOP);
    }
}